...
SWF_delta = 0.1; % Change it for experiment

params = setup_problem_parameters(SWF_delta); 
...
opti.subject_to( n_prime >= 10*log10(params.M' * y + 1e-6) - 10*log10(params.T_ratio) - params.a);

% Cost-Benefit Analysis
opti.subject_to(t == b .* (1 - 0.9 * n_prime / params.Delta_n_max)); % added a coefficient

% SWF
opti.subject_to(u <= t_min_active);
for i = 1:params.n_c
    if ismember(i, params.active_communities_idx) % only for active communities
        opti.subject_to(u(i) <= t(i) - params.Delta);
        opti.subject_to(t_min_active <= t(i));
    end
    if ismember(i, params.inactive_communities_idx) % set values for inactive communities without using in obj func
        opti.subject_to(u(i) == t_min_active - params.Delta);
    end
end

% objective(only for active communities)
opti.subject_to(objective == params.Delta + (1 / params.n_c_active) * sum(u(params.active_communities_idx)));
