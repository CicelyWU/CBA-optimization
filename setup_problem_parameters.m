function params = setup_problem_parameters(delta_input)
    ...
    folder_path = '../input matrix 288/input matrix_od25_r1.2_P1.2/';
    ...
    params.P = sparse(params.P);
    params.P_scaled = sparse(params.P_scaled);
    ...
    params.e =  params.e *0.1;
    params.p = readmatrix(fullfile(folder_path, 'vector_p.xlsx'));
    
    % 查找总需求量大于零的社区索引和数量
    community_total_demand = sum(abs(params.W) .* params.e', 2);     
    params.active_communities_idx = find(community_total_demand > 0);
    params.n_c_active = length(params.active_communities_idx);     
   % 查找无需求社区索引和数量  
    params.inactive_communities_idx = find(community_total_demand == 0);
    params.n_c_inactive = length(params.inactive_communities_idx); 
    
    % Design Parameters
    params.Delta_n_max = 35;
    params.T_ratio = 35.56;
    params.ep = 0.01;
    params.max_energy = 20;
    params.Delta = delta_input; % Example threshold for SWF, adjust as needed
    params.trust_radius_y = 100;
